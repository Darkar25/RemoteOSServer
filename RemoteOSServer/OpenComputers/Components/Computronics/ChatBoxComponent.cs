...
namespace RemoteOS.OpenComputers.Components.Computronics
{
    ...
    public class ChatBoxComponent : Component
    {
        ...
        {
            parent.Listen("chat_message", (parametrs) =>
            {
                if (parametrs[0] == Address.ToString())
                {
                    ChatMessage?.Invoke((Guid)Guid.Parse(parametrs[0]), (string)parametrs[1], (string)parametrs[2]);
                }
            });
        }
        /// <summary>
